...
describe('NewCycleRegisterComponent', () => {
  ...
  it('blocks final new-cycle registration when the client phone does not have 10 digits', () => {
    populateValidSubmissionFields('50 000');
    component.client.phoneNumber = '090745188';
    const alertSpy = spyOn(window, 'alert');
  ...
  it('keeps the existing duplicate phone check for a valid 10-digit number', () => {
    ...
    const callableSpy = functions.httpsCallable as jasmine.Spy;
    const alertSpy = spyOn(window, 'alert');
    component.client.phoneNumber = '0812345678';
    component.allClients = [
      component.client,
      Object.assign(new Client(), {
        uid: 'different-client',
