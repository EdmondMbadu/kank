...
/**
 * PROD — 00:05 Africa/Kinshasa, Monday–Saturday, before collections.
 * Runs for every user EXCEPT those with mode="testing".
 */
exports.scheduleExpectedKinshasaProd = functions
    .runWith({timeoutSeconds: 540, memory: "256MB", maxInstances: 1, failurePolicy: true})
    .pubsub.schedule("5 0 * * 1-6")
    .timeZone("Africa/Kinshasa")
    .onRun(async (context) => {
      const now = new Date();
      const result = await capturePointExpectations({
        db, fieldPath: admin.firestore.FieldPath, now,
        scheduledAt: context.timestamp ? new Date(context.timestamp) : now,
      });
      console.info("Independent point expectations captured", result);
      return result;
    });
...
const WHATSAPP_COPY = {
  ln: {
    ...
    menuHistory: "[3] Historique: Efuteli na ngai",
    menuComplaint: "[4] kotuna mituna",
...
function getWhatsAppParticipantKey(phone) {
  const normalized = normalizeWhatsAppPhone(phone);
  const indexed = phoneToIndexKey(normalized);
  if (indexed) return indexed;
  return normalized.replace(/[^\d]/g, "").trim();
}

function preferWhatsAppDisplayPhone(currentPhone, candidatePhone) {
  const current = normalizeWhatsAppPhone(currentPhone);
  const candidate = normalizeWhatsAppPhone(candidatePhone);
  if (!candidate) return current || "";
  if (!current) return candidate;
  if (candidate.startsWith("+") && !current.startsWith("+")) return candidate;
  if (candidate.length > current.length) return candidate;
  return current;
}

function addWhatsAppParticipant(
    map,
    phone,
    interactionAtMs,
    fullName = "",
  ...
  const safeName = resolvedName.toLowerCase() === "client inconnu" ? "" : resolvedName;
  const lastQuestionAtMs = Number(options.lastQuestionAtMs || 0);
  map.set(key, {
    key,
    phone: preferWhatsAppDisplayPhone(existing.phone, phone) || existing.phone || "",
    fullName: existing.fullName || safeName,
...
function buildWhatsAppParticipantList(
  ...
  return Array.from(participantMap.values())
      ...
      .map((participant) => {
        ...
        const ownerUid = String(phoneIndex.userId || "").trim();
        const clientId = String(phoneIndex.clientId || "").trim();
        const isKnownClient = Boolean(ownerUid && clientId);
        ...
        return {
          ...
          lastQuestionAtMs: isKnownClient ?
            Number(participant.lastQuestionAtMs || 0) :
            0,
          locationName: isKnownClient ?
            String(ownerLocationMap.get(ownerUid) || "").trim() :
            "",
          isKnownClient,
        };
      });
...
async function fetchWhatsAppOwnerLocationMap(phoneIndexMap) {
  const userIds = Array.from(new Set(
      Array.from(phoneIndexMap.values())
          .map((item) => String((item && item.userId) || "").trim())
          .filter(Boolean),
  ));
...
async function listTrackedWhatsAppParticipants() {
  const snap = await db.collection(WHATSAPP_PARTICIPANTS_COLLECTION)
      .select("phone", "fullName", "lastInteractionAtMs", "lastQuestionAtMs")
      .get();
  ...
  const ownerLocationMap = await fetchWhatsAppOwnerLocationMap(phoneIndexMap);
  ...
  for (const participantChunk of chunkArray(participants, 400)) {
    ...
    participantChunk.forEach((participant) => {
      ...
      batch.set(ref, {
        ...
        lastQuestionAtMs: Number(participant.lastQuestionAtMs || 0),
        updatedAt: admin.firestore.FieldValue.serverTimestamp(),
      }, {merge: true});
    });
    await batch.commit();
...
async function buildOverallWhatsAppParticipants(paymentDocs = null) {
  ...
  const [globalMessagePhonesSnap, globalComplaintParticipantsSnap, resolvedPaymentDocs] = await Promise.all([
    db.collection(WHATSAPP_MESSAGES_COLLECTION)
        .select("phone", "createdAtMs", "direction")
        .get(),
...
exports.getWhatsAppAdminReport = functions.https.onCall(async (data, context) => {
  ...
  const allMessages = latestMessagesSnap.docs.map((doc) => {
    ...
    addWhatsAppParticipant(
        ...
        {
          lastQuestionAtMs:
            message.direction === "incoming" ? message.createdAtMs : 0,
        },
    );
    return message;
  });
...
async function logWhatsAppMessage({direction, phone, body, source, extra}) {
  try {
    ...
    await upsertTrackedWhatsAppParticipant({
      ...
      lastQuestionAtMs:
        normalizedDirection === "incoming" ? payload.createdAtMs : 0,
    });
  } catch (err) {
